...
def is_valid_move(board,row,col):
    """
    return True or False indicating of a move or row
...
def player_move(board, symbol): 
    """
    get move from a player with symbol and return updated 
    board after move
    ...
    while not is_valid_move(board, row_choice, col_choice):
        ...
        row_choice = int(input(f'Choose a row positon from 1 to {NUM_ROWS}: '))
        col_choice = int(input(f'Choose a column position from 1 to {NUM_COLS}: '))
